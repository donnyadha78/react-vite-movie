...
const MovieOverview = () => {
  ...
  const { isWatchlist, addToWatchlist } = useWatchlist();
                  ...
                  {movie && (
                    <>
                      <button className="button--primary" onClick={openVideoModal}>
                        ...
                        style={{
                          color: isWatchlist(movie.id) ? '#fff' : getCSSVar('--text-color'),
                          background: isWatchlist(movie.id) ? '#ff2e4f' : 'transparent',
                          border: isWatchlist(movie.id) ? '1px solid #ff2e4f' : `1px solid ${getCSSVar('--text-color')}`
                        }}
                      >
                        {isWatchlist(movie.id) ? 'UnWatchlist' : 'Watchlist'}
                          &nbsp;&nbsp;
                          <i
